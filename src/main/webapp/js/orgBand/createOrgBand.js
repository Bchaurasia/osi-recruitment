app.controller('createOrgBand',['$scope','$http','orgBandService','$state','appConstants','sharedDataService','$timeout',
                                          function($scope, $http, orgBandService, $state, appConstants,sharedDataService,$timeout) {
	$scope.orgBand = [];	
	$scope.org = {};
	$scope.org.bu = "";
	
	$scope.org.selectStream = "";
	$scope.org.selectLevel = "";
	$scope.org.selectGrade = "";
	$scope.org.designation = "";
	...
	$scope.orgObj.level = "";
	$scope.orgObj.grade = "";
	$scope.orgObj.designation = "";
	var designationsPerStream = [];
	
	$scope.duplicateDesignationError = false;
	$scope.streams=[];
	$scope.levels=[];
	$scope.level=[];
	$scope.grades=[];
	...
	$scope.onload = function(){		
		orgBandService.getOrgBands().then(function(data){
			$scope.orgBand = data;
			$scope.arrangeOrgBandData();					
			$timeout( function(){ $scope.message = ""; $scope.cls = ''; sharedDataService.setmessage("");sharedDataService.getClass("");}, 3000);
		}).catch(function(data){
			console.log(data);
		});
		
		if($scope.org.bu==null || $scope.org.bu == ""){
			$scope.orgStreams=true;
			$scope.orgLevel=true;
			$scope.orgGrade=true;
			$scope.orgDesignation=true;
		}
	}
	
	$scope.onload();	
	
	$scope.arrangeOrgBandData = function(){
		for(var i=0;i<$scope.orgBand.length;i++){
			var levelArr = $scope.orgBand[i].levels;
				for(var j=0;j<levelArr.length;j++){
					var designationArr = levelArr[j].designations;
						for(k=0;k<designationArr.length;k++){		
							$scope.orgObj = {};
							$scope.orgObj.bu = $scope.orgBand[i].bu;
							$scope.orgObj.stream = $scope.orgBand[i].stream;
							$scope.orgObj.level = levelArr[j].level;
							$scope.orgObj.grade = designationArr[k].grade;
							$scope.orgObj.designation = designationArr[k].name;
							$scope.orgBandList.push($scope.orgObj);							
						}
				}
		}
	}
	
	$scope.createOrgBand = function() {
			$state.go("admin.orgBand.create");
	}
	
	$scope.clearGrade = function(){
		$scope.org.selectGrade = "";
		$scope.enableDesignation();
	}
	...
	$scope.enableDesignation = function(){
		if($scope.org.selectGrade != null && $scope.org.selectGrade != ""){
			$scope.orgDesignation = false;
		}
		else{
			$scope.orgDesignation = true;
		}
	...
	$scope.validateDesignation = function(){	
		if(designationsPerStream.indexOf($scope.org.designation)!=-1){
			$scope.duplicateDesignationError = true;
		}
		else{
			$scope.duplicateDesignationError = false;
		}
	...
	$scope.range = function (start) {
	  ...
	  $scope.pageCount = function() {
		if (!$scope.orgBandList) { return; }
	    return Math.ceil($scope.orgBandList.length/$scope.itemsPerPage);
	  };
