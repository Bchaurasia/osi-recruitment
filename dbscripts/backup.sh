...
BACKUPDIR="/home/osiuser/Documents/OSIConfigs/mongobackup"
# Mail setup
...
[ ! -z "${1}" ] && [ -f "${1}" ] && source ${1}
#=====================================================================
PATH=/usr/local/bin:/usr/bin:/bin:/usr/bin/mongo
...
mongodump --host=${DBHOST}:${DBPORT} --out=${1} ${OPT} -d $DBNAME
[ -e "${1}" ] && return 0
...
STATUS=1
#remove backup older than one month
find ${BACKUPDIR}/weekly -type f -mtime +30 -exec  rm -f {} \;
find ${BACKUPDIR}/monthly -type f -mtime +30 -exec  rm -f {} \;
find ${BACKUPDIR}/daily -type f -mtime +30 -exec  rm -f {} \;
# Clean up Logfile
eval rm -f "${LOGFILE}"
eval rm -f "${LOGERR}"
exit ${STATUS}
